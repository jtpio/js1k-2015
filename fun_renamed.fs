...
vec2 map(vec3 pos) {
    ...
    for (float i = 1.0; i < 11.1; i+=2.2) {
        vec2 d = abs(vec2(length(vec3(cos(i + T)*pos.x + sin(i + T)*pos.z, pos.y + i, cos(i + T)*pos.z-sin(i + T)*pos.x).xy), cos(i + T)*pos.z-sin(i + T)*pos.x - 40.0 + mod(T * i * 8.0, 90.0))) - vec2(0.7, 5);

        res = opU(res, opU(
                opU(vec2(min(max(d.x,d.y),0.0) + length(max(d,0.0)), 38), vec2(length(vec3(cos(i + T)*pos.x + sin(i + T)*pos.z, pos.y + i, cos(i + T)*pos.z-sin(i + T)*pos.x).yx)-0.5, 3)),
                // the torus along the tube
                vec2(length(vec2(length(vec3(cos(i + T)*pos.x + sin(i + T)*pos.z, pos.y + i, cos(i + T)*pos.z-sin(i + T)*pos.x).xy)-0.9,mod(cos(i + T)*pos.z-sin(i + T)*pos.x,4.0)-2.0))-0.2, 5))
              );
    }
    return res;
}
