...
vec2 map(vec3 pos) {
    ...
    for (float i = 1.0; i < 9.9; i+=1.9) {
        ...
        res = opU(res, opU(
                ...
                vec2(length(vec2(length(vec3(cos(i + iGlobalTime * 0.3)*pos.x + sin(i + iGlobalTime * 0.3)*pos.z, pos.y + i, cos(i + iGlobalTime * 0.3)*pos.z-sin(i + iGlobalTime * 0.3)*pos.x).xy)-0.9,mod(cos(i + iGlobalTime * 0.3)*pos.z-sin(i + iGlobalTime * 0.3)*pos.x,4.0)-2.0))-0.2, 5))
              );
    }
    return res;
}
