...
vec2 map(vec3 pos) {
    ...
    for (float i = 0.3; i < 1.0; i+=0.1) {
        vec3 p = vec3(cos(i * 99.0 + iGlobalTime / 3.0)*pos.x + sin(i * 99.0 + iGlobalTime / 3.0)*pos.z, pos.y, -sin(i * 99.0 + iGlobalTime / 3.0)*pos.x + cos(i * 99.0 + iGlobalTime / 3.0)*pos.z) + vec3(2.0, sqrt(i) * 40.0 - 40.0, 1.0);
